...
The project ingests the provided data, generate events into a message queue, consumes messages from a queue and finally
generates the results of the analysis. The source data is in JSON format, is converted into a format easily consumed by
the messaging system(Kafka) and the Apache Spark Streaming engine consumes these messages, performs some transformation
and aggregation before returning results that answer the 2 important question of the project:
...
* Ensure the environment variables are set on the system. For Unix-based system, .bash_profile is the target while
  Windows based systems have PATHs

 ```shell
 # Examples for Unix-based operation systems
...
* Perform data transformation and analytics by running the data_stream.py file.   
  ...
  For the sake of this project, the transformation/analytics results is display on the console. Below is the screenshot of the progress report.

![Streaming progress report](images/Streaming_Progress_Report.png "Progress Report")

A future upgrade can write the results to a Kafka topic that can then serve as a data source for a visualization tool.
That is out of scope of this project. In the meantime, you can monitor the progress of the report visually by visiting http://localhost:3000 while the transformation is in progress. It should look like
  this:
...
__1. How did changing values on the SparkSession property parameters affect the throughput and latency of the data?__

There is a correlation between the Spark config parameter  ```maxOffsetsPerTrigger``` and
the ```processedRowsPerSecond  and numInputRows```. The higher the ```maxOffsetsPerTrigger``` , the higher the values
of ```processedRowsPerSecond  and numInputRows```  which is the throughput. The screenshots below:
...
After changing the processedRowsPerSecond changed so if we have a higher value in this value the throughput is better.
Also the numInputRows changed.

__2. What were the 2-3 most efficient SparkSession property key/value pairs? Through testing multiple variations on values, how can you tell these were the most optimal?__
Combining two configuration options (maxOffsetsPerTrigger and maxRatePerPartition) shows a relationship that can give optimal performance.
...
A third configuration option that makes Spark Structure stream resilient is the  **checkpointLocation** which allows
Apache Spark recover quickly in case of restarts or operation mishaps.

## Help

While running Kafka in single mode if you encounter
the ```message brokers not meeting the required replication factor of 3.``` error. Simply to
add ```offsets.topic.replication.factor=1```
